...
const textAreaStates: textAreaState[] = [
  "default",
  "error",
  "warning",
  "disabled",
  ...
  describe("testes de estilização", () => {
    it.each(
      textAreaStates.flatMap((state) =>
        requiredValues.map(
          (required) => [state, required] as [textAreaState, boolean],
        ),
      ),
    )(
      ...
      (state, required) => {
        ...
        const label = getByText(
          `TextArea ${state} ${required ? "required" : "optional"}`,
          { exact: false },
        );
