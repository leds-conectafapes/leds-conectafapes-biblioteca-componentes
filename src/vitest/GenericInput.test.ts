...
describe("GenericInput.vue", () => {
    ...
    it.each(
      inputTypes.flatMap((type) =>
        inputStates.flatMap((state) =>
          requiredValue.map(
            (required) =>
              [type, state, required] as [inputType, inputState, boolean],
          ),
        ),
      ),
    )(
      ...
      (type, state, required) => {
        ...
        const input = getByLabelText(
          `Input ${state} ${type} ${required ? "required" : "optional"}`,
          { exact: false },
        );
  ...
  describe("testes de comportamento", () => {
    ...
    it.each(inputTypes)('escrita no campo de input com type "%s"', (type) => {
      ...
      const getLastEmittedValue = () => {
        const events = emitted()["update:modelValue"] as
          | unknown[][]
          | undefined;
    ...
    it.each(inputTypes)('envio de formulario com type "%s"', (type) => {
      ...
      if (type === "email") {
        fireEvent.update(input, "invalid-email");
        fireEvent.click(button);
        expect(onSubmit).not.toHaveBeenCalled();

        fireEvent.update(input, "a@b.com");
        fireEvent.click(button);
      ...
      if (type === "url") {
        fireEvent.update(input, "invalid-url");
        fireEvent.click(button);
        expect(onSubmit).not.toHaveBeenCalled();

        fireEvent.update(input, "https://example.com");
        fireEvent.click(button);
