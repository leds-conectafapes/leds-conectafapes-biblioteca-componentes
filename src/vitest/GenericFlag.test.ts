...
import GenericFlag from '../components/GenericFlag/GenericFlag.vue'

import type { flagVariant } from '../types'

const flagVariants: flagVariant[] = ['informative', 'success', 'warning', 'error']
...
const flagStyleMap: Record<flagVariant, {
  card_border: string
  card_bg: string
  icon: string
  title: string
  ...
  describe('testes de estilização', () => {
    ...
    it.each(
      flagVariants.flatMap(variant =>
        activeValues.map(active => [variant, active] as [flagVariant, boolean])
      )
    )(
      'estilização correta com variant "%s" e active %s',
      (variant, active) => {
        ...
        expect(description).toHaveTextContent(`Snackbar Description - ${variant} ${active}`)
